...
const { op_user_worker_fetch_send, op_user_worker_create } = ops;
...
class UserWorker {
  ...
  async fetch(request, options = {}) {
    ...
    const [requestBodyPromiseResult, responsePromiseResult] =
      await Promise.allSettled([requestBodyPromise, responsePromise]);
  ...
  static async create(opts) {
    const readyOptions = {
      ...
      cpuTimeSoftLimitMs: 500,
      cpuTimeHardLimitMs: 1000,
      noModuleCache: false,
      importMapPath: null,
      envVars: [],
      forceCreate: false,
